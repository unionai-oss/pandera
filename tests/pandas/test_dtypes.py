...
def test_category_mismatch_error_reports_categories():
    """Categorical dtype mismatches should report the differing categories.

    Regression test for #2051: a column whose categories are a subset of the
    schema's categories failed with the unhelpful message
    "expected series 'col' to have type category, got category" because both
    dtypes stringify to "category".
    """
    schema = pa.DataFrameSchema(
        {"col": pa.Column(pd.CategoricalDtype(["a", "b"]))}
    )
    df = pd.DataFrame(
        {"col": pd.Series(["a", "a"], dtype=pd.CategoricalDtype(["a"]))}
    )
    with pytest.raises(pa.errors.SchemaError) as exc_info:
        schema.validate(df)

    message = str(exc_info.value)
    assert "with categories ['a', 'b']" in message
    assert "with categories ['a']" in message
