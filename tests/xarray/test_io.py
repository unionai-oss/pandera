...
from pandera.schema_statistics.xarray import parse_checks
...
class TestNamedCheckSerialization:
    ...
    def test_parse_checks_keeps_registered_name(self):
        check = Check.gt(0, name="positive")
        [serialized] = parse_checks([check])

        assert serialized["min_value"] == 0
        assert serialized["options"]["check_name"] == "greater_than"
        assert serialized["options"]["name"] == "positive"

    @pytest.mark.parametrize(
        "serialize, deserialize",
        [(to_yaml, from_yaml), (to_json, from_json)],
    )
    def test_data_array_named_check_survives_roundtrip(
        self, serialize, deserialize
    ):
        schema = DataArraySchema(
            dtype="int64",
            dims=("x",),
            checks=Check.gt(0, name="positive"),
        )
        payload = serialize(schema, minimal=False)
        assert "greater_than" in payload

        restored = deserialize(payload)
        [check] = restored.checks
        assert check.name == "positive"
        assert check.statistics == {"min_value": 0}
        with pytest.raises(SchemaError):
            restored.validate(xr.DataArray(np.array([-1]), dims=("x",)))
        restored.validate(xr.DataArray(np.array([1]), dims=("x",)))

    def test_dataset_component_named_checks_survive_roundtrip(self):
        schema = DatasetSchema(
            data_vars={
                "v": DataVar(
                    dtype="int64",
            ...
            coords={
                "x": Coordinate(
                    dtype="int64", checks=Check.ge(0, name="nonneg")
                )
            },
        )
        restored = from_json(to_json(schema, minimal=False))

        [data_var_check] = restored.data_vars["v"].checks
        assert data_var_check.name == "positive"
        [coord_check] = restored.coords["x"].checks
        assert coord_check.name == "nonneg"
        with pytest.raises(SchemaError):
            restored.validate(
                xr.Dataset(
                    {"v": (("x",), np.array([-1]))},
