        ...
        if schema.dtype is not None:
            ...
            if isinstance(dtype_check_results, bool):
                ...
                if not passed and schema.dtype is not None:
                    ...
                    failure_cases = str(check_obj.dtype)
                expected_str = str(schema.dtype)
                actual_str = str(check_obj.dtype)
                if expected_str == actual_str:
                    # Some dtypes (e.g. pandas categoricals) share a string
                    # representation but differ in their parameters. Surface
                    # the distinguishing categories so the message is
                    # actionable instead of "got category, expected category".
                    expected_cats = getattr(schema.dtype, "categories", None)
                    actual_cats = getattr(check_obj.dtype, "categories", None)
                    if expected_cats is not None and actual_cats is not None:
                        expected_str = (
                            f"{expected_str} with categories "
                            f"{list(expected_cats)}"
                        )
                        actual_str = (
                            f"{actual_str} with categories {list(actual_cats)}"
                        )
                msg = (
                    f"expected series '{check_obj.name}' to have type "
                    f"{expected_str}, got {actual_str}"
                )
            else:
