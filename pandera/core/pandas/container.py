...
from pandera.backends.pandas.base import PandasSchemaBackend, ContainerInfo
...
class PandasSchemaContainerBackend(PandasSchemaBackend[pd.DataFrame]):
    ...
    def validate(
        ...
        *,
        ...
        container_info = self.collect_container_info(check_obj, schema)

        # check the container metadata, e.g. field names
        self.check_column_names_are_unique(check_obj, schema)
        self.check_column_presence(check_obj, schema, container_info)
        ...
        schema_components = self.collect_schema_components(
            check_obj, schema, container_info
        )
    ...
    def collect_container_info(self, check_obj: pd.DataFrame, schema) -> ContainerInfo:
    ...
    def collect_schema_components(
        self, check_obj: pd.DataFrame, schema, container_info: ContainerInfo,
    ):
        schema_components = []
        for col_name, col in schema.columns.items():
            if (
                col.required or col_name in check_obj
            ) and col_name not in container_info.lazy_exclude_column_names:
    ...
    def strict_filter_columns(
        ...
        for column in container_info.destuttered_column_names:
            is_schema_col = column in container_info.expanded_column_names
            ...
            if schema.ordered and is_schema_col:
                try:
                    next_ordered_col = next(container_info.sorted_column_names)
